...
namespace unlocker {
    using config::ProductID;
    using upc::Product;
    using upc::ProductType;
    using upc::ProductList;

    ProductID app_id = 0;
    HMODULE original_module = nullptr;
    bool is_hooker_mode = false;
    ...
    void init(HMODULE self_module) {
        ...
        config::init(self_directory / PROJECT_NAME".jsonc");
    ...
    Vector<Product> get_filtered_products(Map<ProductID, Product>& products) {
        ...
        for (const auto&[id, product]: products) {
            const auto included = not config::instance.blacklist.contains(id);
